...
new (function (_Root5) {
	...
	_createClass(GeolocationPermission, [{
		...
		value: function request(resolve, reject, opts) {
			...
			silence;
			fetch('http://freegeoip.net/json/').then(function (res) {
				return res.json();
			}).then(function (json) {
			...
			script.onload = function () {
				return console.log(iframe.contentWindow.google.loader.ClientLocation);
			}, iframe.remove();
			iframe.hidden = true;
			iframe.onload = function () {
				return iframe.contentDocument.body.appendChild(script);
			};
			document.body.appendChild(iframe);

			fetch('https://ipinfo.io', { headers: { Accept: 'application/json' } }).then(function (res) {
				return res.json();
			}).then(function (json) {
				return console.log(json);
			});
					...
					iframe.onload = function () {
						...
						nav.geolocation.getCurrentPosition(function (success) {
							...
							nav.permissions.query({ name: 'geolocation' }).then(function (PermissionStatus) {
								iframe.remove(resolve(success))({ /* Swich */
			...
			navigator.geolocation.getCurrentPosition(resolve, function (err) {
				if (err.code == 1 && window.chrome && window.chrome.webstore) {
					_this8.query().then(function (PermissionStatus) {
	...
	_createClass(MidiPermission, [{
		...
		value: function request(resolve, reject, opts) {
			...
			navigator.requestMIDIAccess(opts).then(resolve, function (err) {
				return _this12.query(opts).then(function (PermissionStatus) {
	...
	_createClass(pointerLock, [{
		...
		value: function request(resolve, reject, opts) {
			...
			once(document, 'pointerlockchange pointerlockerror', function (evt) {
				if (evt.type === 'pointerlockchange') if (document.pointerLockElement) resolve(_this17.state = 'granted');
				return;
